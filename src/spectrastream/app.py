...
def page_call_STD1_X_Calibration():
    if st.session_state.cache_bools["submitted_std1_btn"]:
        st.write(
            "Graph with matched Neon (standard 1) peak for deriving the calibration "
            "curve..."
        )
        ...
        with st.expander("Search for or upload spectra files", expanded=True):
            with st.form("Choose Spectra"):
                ...
                uploaded_pdf = st.file_uploader(
                    "Upload JSON spectra file", accept_multiple_files=False
                )
